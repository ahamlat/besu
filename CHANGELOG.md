...
- Speed up state reads (SLOAD and account access): share the storage-slot keccak memoization across transactions and blocks, prefetch the block's working set before execution (from the block access list when present, otherwise from transaction senders/recipients/access lists), batch same-account slot reads with RocksDB MultiGet, and enable memtable bloom filters plus data-block hash indexes on the world-state column families
